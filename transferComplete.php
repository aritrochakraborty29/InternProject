...
<body>
	...
	<?php 
		...
		if( $sender != $receiver && $transfer_amount>0) {
				...
				echo "<h5>₹{$transfer_amount} has been deducted from your account i.e. {$sender} and the amount is succesfully transfered to {$receiver}.</h5>";
				echo "<a href=\"transferHistory.php\" class=\"btn-1\">Transfer History</a>";
				echo "     <a href=\"index.php\" class=\"btn-1\">Home</a>";
		}
		else {
			echo "<h3 class=\"red-text accent-3\"> Transaction Failed!</h3>";
			if($sender==$receiver) {
				
				echo "<h5 class=\"red-text accent-3\">Sender & Reciever Cannot Be Same!.</h5>";
			}
			else {
				echo "<h5 class=\"red-text accent-3\">Transfer Amount Cannot Be Negative!.</h5>";
			}
			echo "<p>Please wait redirecting to previous page. <a href=\"transfer.php\">Click here</a> to redirect manually</p>";
